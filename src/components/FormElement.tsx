...
export const FormElement = ({ id, field }: FormElementProps) => {
    const {
        activeId,
        fields,
        updateFields,
    ...
    const removeFieldFromArray = (
        fields: FormElementProps["field"][],
        fieldId: string
    ): FormElementProps["field"][] => {
        return fields
    ...
    return (
        <div
            ...
            {...listeners}
            onClick={handleClick}
            className={`bg-white rounded-lg shadow-sm border border-gray-200 ${isLayout ? "p-4" : "p-4"} ${
        ...
        >
            ...
            {isLayout && (
                <div className='mt-4'>
                    <div
                        className={`border-2 border-dashed border-gray-200 rounded-lg p-4 min-h-[100px] ${
                            isHorizontalLayout ? "flex flex-row items-stretch" : "space-y-2"
                        }`}
                    >
                        {field.elements && field.elements.length > 0 ? (
                            <div
                                className={`${isHorizontalLayout ? "flex flex-row items-stretch flex-1" : "space-y-2"}`}
                            >
                                ...
                                {field.elements.map((element, index) => (
                                    <div
                                        key={element.id}
                                        className={isHorizontalLayout ? "flex flex-row items-stretch" : ""}
                                    >
